...
// Redefine the macros for uint8_t-swapping, to sure the correct
...
inline int ProtoMsguint8_tSize( const TMsg &msg )
{
	#if GOOGLE_PROTOBUF_VERSION < 3004000
		return msg.uint8_tSize();
	#else
		return static_cast<int>( msg.uint8_tSizeLong() );
	#endif
}
...
/// It would be nice to use a smaller tag, but BCrypt requires a 16-uint8_t tag,
...
COMPILE_TIME_ASSERT( ( (k_usecMaxAckStatsDelay*2) >> k_usecAckDelayPacketSerializedPrecisionShift ) < 0x4000 ); // Make sure we varint encode in 2 uint8_ts, even if we overshoot a factor of 2x
...
// Serialize an UNSIGNED quantity.  Returns pointer to the next uint8_t.
// https://developers.google.com/protocol-buffers/docs/encoding
template <typename T>
inline uint8_t *SerializeVarInt( uint8_t *p, T x )
{
...
template <typename T>
inline uint8_t *SerializeVarInt( uint8_t *p, T x, const uint8_t *pEnd )
{
	while ( x >= (unsigned)0x80 ) // if you get a warning, it's because you are using a signed type!  Don't use this for signed data!
	{
		if ( p >= pEnd )
...
// De-serialize a var-int encoded quantity.  Returns pointer to the next uint8_t,
// or NULL if there was a decoding error (we hit the end of stream.)
// https://developers.google.com/protocol-buffers/docs/encoding
//
// NOTE: We do not detect overflow.
template <typename T>
inline uint8_t *DeserializeVarInt( uint8_t *p, const uint8_t *end, T &x )
{
	if ( p >= end )
		return nullptr;
...
// Const version
template <typename T>
inline const uint8_t *DeserializeVarInt( const uint8_t *p, const uint8_t *end, T &x )
{
	return DeserializeVarInt( const_cast<uint8_t*>( p ), end, x );
}
...
inline bool AddrEqual( const SteamNetworkingIPAddr &s, const netadr_t &n )
{
	...
	switch ( n.GetType() )
	{
		...
		case k_EIPTypeV6:
			return memcmp( s.m_ipv6, n.GetIPV6uint8_ts(), 16 ) == 0;
	}
...
extern bool BSteamNetworkingIdentityToProtobufInternal( const SteamNetworkingIdentity &identity, std::string *strIdentity, std::string *uint8_tsMsgIdentityLegacyBinary, SteamDatagramErrMsg &errMsg );
...
extern bool BSteamNetworkingIdentityFromLegacyBinaryProtobuf( SteamNetworkingIdentity &identity, const std::string &uint8_tsMsgIdentity, SteamDatagramErrMsg &errMsg );
...
extern GlobalConfigValue<int32> g_Config_PacketTraceMaxuint8_ts;
