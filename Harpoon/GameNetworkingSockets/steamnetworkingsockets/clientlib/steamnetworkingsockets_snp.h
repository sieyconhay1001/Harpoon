...
// If app tries to send a message larger than N uint8_ts unreliably,
...
public:
	...
	/// Offset in reliable stream of the header uint8_t.  0 if we're not reliable.
	...
	uint8_t *SNPSend_ReliableHeader()
	{
		// !KLUDGE! Reuse the peer identity to hold the reliable header
		return (uint8_t*)&m_identityPeer;
	}
...
struct SNPRange_t
{
	/// uint8_t or sequence number range
	int64 m_nBegin;
...
{
	/// Current sending rate in uint8_ts per second, RFC 3448 4.2 states default
...
struct SSNPSenderState
{
	...
	/// How many uint8_ts into the first message in the queue have we put on the wire?
	...
	inline int Pendinguint8_tsTotal() const { return m_cbPendingUnreliable + m_cbPendingReliable; }
...
struct SSNPReceiverState
{
	...
	/// Stream position of the first uint8_t in m_bufReliableData.  Remember that the first uint8_t
	...
	std_vector<uint8_t> m_bufReliableStream;
