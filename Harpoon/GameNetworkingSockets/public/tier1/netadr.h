...
extern const uint8_t k_ipv6uint8_ts_LinkLocalAllNodes[16];
extern const uint8_t k_ipv6uint8_ts_Loopback[16];
extern const uint8_t k_ipv6uint8_ts_Any[16];
...
public:
	...
	/// Set IPv4 IP given host-uint8_t order argument.
	...
	void	SetIPV6Broadcast( uint32 nScope = 0 ) { SetIPV6( k_ipv6uint8_ts_LinkLocalAllNodes, nScope ); }
	...
	void	SetIPV6Loopback() { SetIPV6( k_ipv6uint8_ts_Loopback, 0 ); }
	...
	void	SetIPV6Any() { SetIPV6( k_ipv6uint8_ts_Any, 0 ); }
	...
	/// Get the IPv4 IP, in host uint8_t order.  Should only be called on IPv4 addresses.
	/// For historical reasons, this can be called on an NA_NULL address, and usually
	/// will return 0.
	uint	GetIPv4() const;

	/// Fetch port (host uint8_t order)
	unsigned short GetPort() const { return port; }

	/// Get IPv6 uint8_ts.  This will work on any address type
	/// An NA_NULL address returns all zeros.
	/// For the IPv4 address aa.bb.cc.dd, we will return ::ffff:aabb:ccdd
	void GetIPV6( uint8_t *result ) const;

	/// Get pointer to IPV6 uint8_ts.  This should only be called on IPv6
	/// address, will assert otherwise
	const uint8_t *GetIPV6uint8_ts() const;

	/// Set IPv6 address (as 16 uint8_ts) and scope.
	/// This slams the address type to NA_IPV6, but
	/// does not alter the port.
	void SetIPV6( const uint8_t *uint8_ts, uint32 nScope = 0 );

	/// Set IPv6 address (as 16 uint8_ts), port, and scope.
	/// This also sets the address type to NA_IPV6.
	void SetIPV6AndPort( const uint8_t *uint8_ts, uint16 nPort, uint32 nScope = 0 ) { SetIPV6( uint8_ts, nScope ); SetPort( nPort ); }
...
private:
	...
	union {
		...
		uint8_t			ipuint8_t[4];		// IP stored in host order (little-endian)
		struct
		{
		...
		// IPv6
		//
		uint8_t			ipv6uint8_t[16];	// Same as inaddr_in6.  (0011:2233:4455:6677:8899:aabb:ccdd:eeff)
...
inline const uint8_t *netadr_t::GetIPV6uint8_ts() const
{
	Assert( type == NA_IPV6 );
	return ipv6uint8_t;
}

inline void netadr_t::SetIPV6( const uint8_t *uint8_ts, uint32 nScope )
{
	type = NA_IPV6;
	memcpy( ipv6uint8_t, uint8_ts, 16 );
	ipv6Scope = nScope;
}
