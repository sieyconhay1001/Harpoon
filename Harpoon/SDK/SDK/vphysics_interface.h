...
#include "IPanoramaUIEngine.h" // Move IAPPSystem Out Of Here
...
public:
	...
	virtual Vector			CollideGetExtent(const CPhysCollide* pCollide, const Vector& collideOrigin, const Angle& collideAngles, const Vector& direction) = 0;

	// Get an AABB for an oriented collision model
	virtual void			CollideGetAABB(Vector* pMins, Vector* pMaxs, const CPhysCollide* pCollide, const Vector& collideOrigin, const Angle& collideAngles) = 0;
	...
	virtual void TraceBox(const Vector& start, const Vector& end, const Vector& mins, const Vector& maxs, const CPhysCollide* pCollide, const Vector& collideOrigin, const Angle& collideAngles, trace_t* ptr) = 0;
	virtual void TraceBox(const Ray_t& ray, const CPhysCollide* pCollide, const Vector& collideOrigin, const Angle& collideAngles, trace_t* ptr) = 0;
	virtual void TraceBox(const Ray_t& ray, unsigned int contentsMask, IConvexInfo* pConvexInfo, const CPhysCollide* pCollide, const Vector& collideOrigin, const Angle& collideAngles, trace_t* ptr) = 0;

	// Trace one collide against another
	virtual void TraceCollide(const Vector& start, const Vector& end, const CPhysCollide* pSweepCollide, const Angle& sweepAngles, const CPhysCollide* pCollide, const Vector& collideOrigin, const Angle& collideAngles, trace_t* ptr) = 0;
...
public:
	...
	virtual void Update(const Vector & position, const Angle & angles, float timeOffset) = 0;
	...
	virtual float GetTargetPosition(Vector* pPositionOut, Angle* pAnglesOut) = 0;
...
public:
	...
	virtual IPhysicsObject* CreatePolyObject(const CPhysCollide* pCollisionModel, int materialIndex, const Vector& position, const Angle& angles, objectparams_t* pParams) = 0;
	// same as above, but this one cannot move or rotate (infinite mass/inertia)
	virtual IPhysicsObject* CreatePolyObjectStatic(const CPhysCollide* pCollisionModel, int materialIndex, const Vector& position, const Angle& angles, objectparams_t* pParams) = 0;
	// Create a perfectly spherical object
	virtual IPhysicsObject* CreateSphereObject(float radius, int materialIndex, const Vector& position, const Angle& angles, objectparams_t* pParams, bool isStatic) = 0;
	...
	virtual void SweepCollideable(const CPhysCollide* pCollide, const Vector& vecAbsStart, const Vector& vecAbsEnd,
		const Angle& vecAngles, unsigned int fMask, IPhysicsTraceFilter* pTraceFilter, trace_t* pTrace) = 0;
...
public:
	...
	virtual void			SetPosition(const Vector& worldPosition, const Angle& angles, bool isTeleport) = 0;
	virtual void			SetPositionMatrix(const matrix3x4_t& matrix, bool isTeleport) = 0;

	virtual void			GetPosition(Vector* worldPosition, Angle* angles) const = 0;
	...
	virtual void			UpdateShadow(const Vector& targetPosition, const Angle& targetAngles, bool tempDisableGravity, float timeOffset) = 0;

	// returns number of ticks since last Update() call
	virtual int				GetShadowPosition(Vector* position, Angle* angles) const = 0;
